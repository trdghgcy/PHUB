...
def main(input, output, quality, downloader):
    ...
    if len(urls) > 1 and os.path.isfile(output):
        raise Exception(
            'Must specify directory when downloading multiple videos.')
    client = phub.Client(delay=0.02)
    for url in urls:
        try:
            video = client.get(url.strip())
            video.download(path=output, quality=quality, downloader=downloader,
